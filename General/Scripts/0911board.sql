...
-- 계정 생성
CREATE USER SPRING_JSH IDENTIFIED BY SPRING1234;

-- 권한 부여
GRANT CONNECT, RESOURCE TO SPRING_JSH;

-- 테이블 저장 공간 할당
ALTER USER SPRING_JSH
DEFAULT TABLESPACE USERS
QUOTA 100M ON USERS;*/

-----------------------------------------------

/* SPRING 계정 접속 후 테이블, 시퀀스 생성 구문 */
/* 회원 "MEMBER" */
CREATE TABLE "MEMBER"(
	"MEMBER_NO" NUMBER,
	"MEMBER_EMAIL" VARCHAR2(50) NOT NULL,
	"MEMBER_PW" VARCHAR2(100) NOT NULL,
	"MEMBER_NICKNAME" NVARCHAR2(10) NOT NULL, -- 10글자 저장
	"MEMBER_TEL" CHAR(11) NOT NULL,
	"MEMBER_ADDRESS" NVARCHAR2(150),
	"PROFILE_IMG" VARCHAR2(300), -- 파일 경로 저장
...
UPDATE "MEMBER"
SET 
"MEMBER_PW" = '$2a$10$ii.OR3lI1dBtD0cOHO/lNOixXPa2CdHKXoKDj8hF8GBAz1/TvrTgi'
WHERE
"MEMBER_NO" = 1;
...
COMMENT ON COLUMN "MEMBER"."MEMBER_NICKNAME" IS '회원 닉네임(별명)';
...
COMMENT ON COLUMN "BOARD"."BOARD_DEL_FL" IS '삭제여부 (Y, N)';
...
COMMENT ON COLUMN "BOARD_IMG"."BOARD_NO" IS '이미지 첨부된 게시글 번호';
...
COMMENT ON COLUMN "BOARD_LIKE"."BOARD_NO" IS '좋아요 눌린 게시글 번호';
...
COMMENT ON COLUMN "COMMENT"."PARENT_COMMENT_NO2" IS '부모 댓글번호';
 ...
 * 1. INSERT ALL 구문
 * 2. INSERT + SUB QUERY (2번 사용)
 * 
 * [문제점] 위의 두 방법 다 SERQUENCE 직접 사용 불가!!
 * */
...
SELECT
	...
	(SELECT IMG_PATH || IMG_RENAME
	FROM BOARD_IMG I
	WHERE I.BOARD_NO = 2006) AS THUMBNAIL
FROM "BOARD" B
...
/*특정 게시글 댓글 목록 조회 + 계층형 쿼리 이용*/
-- LEVEL 컬럼 : 현재 계층의 레벨 출력
SELECT LEVEL, COMMENT_NO, PARENT_COMMENT_NO2, COMMENT_CONTENT
FROM "COMMENT"
WHERE BOARD_NO = 2009
/* 계층형 쿼리 코드 추가 */
-- PARENT_COMMENT_NO 값이 NULL인 행이 계층의 시작이다 (ROOT)
-- LV.1 로 지정
...
-- 형제들(같은 Lv) 간의 정렬 순서를 COMMENT_NO 오름차순
