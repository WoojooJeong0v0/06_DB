...
-- 서브쿼리 이용해서 복사할 경우 RESULT SET을 복사하는 것임
-- 제약조건이니 컬럼 설명이 없어서 NOT NULL 빼고는 복사가 안 됨!
-- NOT NULL은 성질을 바꾸는 것이라 복사 가능
...
ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE NOT NULL; -- NULL 허용 안 함
...
-- 예 ) CHAR (3) 짜리 컬럼에 'ABC' 저장된 상태
			-- > CHAR (1) 로 수정하려고 하면 오류 발생 
...
SELECT * FROM DEPT_COPY;
-- > 새로 추가된 LNAME 컬럼 기본값 '한국'이 자동으로 세팅됨
...
-- 테이블 정의가 아니라 컬럼 값을 수정하는 것이므로 DML, UPDATE로 해야 함
...
-- DEPT_COPY TABLE에 DEPT_ID 컬럼 값으로 'D0' 추가
-- INSERT DML 수행 
...
-- ROLLBACK 수행해서 트랜잭션 저장된 DML 다 지우기
ROLLBACK;
...
SELECT * FROM DEPT_COPY; -- D0 삽입완료

-- DEPT_COPY 테이블에 컬럼 추가 DDL 수행
ALTER TABLE DEPT_COPY
ADD (DEPT_TITLE VARCHAR2(30));

SELECT * FROM DEPT_COPY; -- DEPT_TITLE 추가완료

ROLLBACK;

SELECT * FROM DEPT_COPY; -- 삭제되지 않았다!!
-- > ALTER (DDL) 수행 시 트랜잭션이 자동으로 COMMIT 됨!

--> 결론 : DML/DDL 혼용해서 사용하지 말자!!!
-- 내가 원할 때 커밋/롤백 사용할 수 없으므로 웬만하면 혼용사용하지 말자!
-- 롤백은 데이터 무결성을 위해 무척 중요함!!
...
-- TB2 테이블 생성 (TB1과 관계설정, FK 제약 조건 하겠다)
CREATE TABLE TB2(
	TB2_PK NUMBER PRIMARY KEY,
	TB2_COL NUMBER 
		REFERENCES TB1(TB1_PK) -- TB1_PK에 있는 값만 작성되게 하겠다
);

-- TB1(참조 당하고 있는 부모) 테이블 삭제
DROP TABLE TB1; 
-- SQL Error [2449] [72000]: ORA-02449: 외래 키에 의해 참조되는 고유/기본 키가 테이블에 있습니다
-- PK 컬럼이 다른 테이블에 참조 당하고 있다는 의미

-- 해결 방법1 ) 참조하는 자식 테이블(TB2) 를 먼저 삭제하거나 FK 제약조건 삭제 

-- 해결 방법2 ) CASCADE CONSTRAINTS 옵션 사용하기
-- FK 제약조건 삭제 
-- > 테이블 삭제 시 연결되어 있는 FK 제약조건을 같이 삭제 시킴
DROP TABLE TB1 CASCADE CONSTRAINTS;
-- TB1 테이블 삭제
-- + TB2 FK 제약 조건도 같이 삭제 됨 (관계를 끊어버림)
