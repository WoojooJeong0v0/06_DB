...
/*3*/SELECT DEPT_CODE, SUM(SALARY) "급여합" -- 7개의 그룹별로 합을 구해라
/*1*/FROM EMPLOYEE
/*2*/GROUP BY DEPT_CODE; -- 같은 DERT_CODE끼리 그룹
-- 7종류의 DEPT_CODE가 존재
...
SELECT DEPT_CODE, SUM(SALARY) "급여 합계", AVG(SALARY) "급여 평균", COUNT(*) "인원 수"
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
-- 부서코드 순으로 정렬
SELECT DEPT_CODE, COUNT(*)    -- 4. 조회결과에 포함될 컬럼 지정
FROM EMPLOYEE									-- 1. 어떤 테이블에서
WHERE BONUS IS NOT NULL       -- 2. 이 조건을 만족하는 행을 고르겠다 (어떤 행만 조회할지)
GROUP BY DEPT_CODE						-- 3. WHERE에서 추려진 행에서 결과 그룹 구분
...
SELECT JOB_CODE, FLOOR((SUM(SALARY))) "급여 합"
FROM EMPLOYEE
...
-- 부서 코드 오름차순으로 정렬
-- 부서를 먼저 묶고 직급을 다음으로 묶음

SELECT DEPT_CODE, JOB_CODE, SUM(SALARY) "급여 합", COUNT(*)
FROM EMPLOYEE
 ...
 * WHERE와 HAVING 의 차이
 * 
 * WHERE : FROM 절에서 지정된 테이블 전체 행 중 조건에 맞는 행만 골라내는 구문
 * HAVING : GROUP BY 를 통해 만들어진 그룹 중
 * 					조건에 맞는 그룹만 골라내는 구문
 * */
...
-- WHERE로 하면 개인별 급여가 350만 이상인 사람들의 부서별 평균
-- 조건에 맞는 구문이 아니다!!! (잘못된 SQL)
...
ORDER BY DEPT_CODE ASC;
-- 일단 그룹으로 묶어라, 그 7개의 그룹의 평균을 별도로 구하는 것!!
--> EMPLOYEE 테이블에서 DEPT_CODE 기준으로 만들어진 그룹 중
-- 그룹별 급여 평균이 350만 이상인 그룹만 조회하겠다 라는 의미
...
GROUP BY JOB_CODE
ORDER BY JOB_CODE ASC; -- 4행 조회 (0명은 조회결과에 포함 안 됨)

-- COUNT(컬럼명) : 해당 컬럼에 값이 있는 행이 몇 개인지 셈
--> NULL이면 카운트에서 제외
-- 모든 행이 다 보여지는 상태 
...
/*통계적 측면*/
-- 통계에서는 0명도 포함시키는 경우가 있어서 아래 쪽이 좀더 좋은 방법!
...
-- 급여가 400만 초과하는 사원 이름, 부서코드, 급여 조회
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
--> 교집합인 심봉선, 대북혼을 제외한 앞쪽 SELECT 의 RESULT SET의 결과가 조회됨
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
MINUS
