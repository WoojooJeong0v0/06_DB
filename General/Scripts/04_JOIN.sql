...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
		--> DEPT_CODE, DEPT_ID 값이 같은 행을 한 행으로 합쳐라
...
FROM EMPLOYEE JOIN DEPARTMENT -- ANSI 방식 : JOIN이란 단어가 들어감
ON (DEPT_CODE = DEPT_ID);
-- > DEPT_CODE, DEPT_ID가 일치하는 행끼리 한 행으로 합쳐짐
--  > 단, 일치하는 값이 없을 경우 합쳐진 JOIN 결과에서 제외 된다
		--> == INNER JOIN


-- 오라클 (JOIN이라는 단어를 작성하지 않음)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_ID, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT -- ANSI 방식 : JOIN이란 단어가 들어감
...
-- INNER JOIN이 JOIN의 기본값이라 생략 가능함
...
--> JOB_CODE 컬럼명이 같아서 어떤 테이블 컬럼인지 구분할 수 없어서 나는 오류 
-- EMPLOYEE E : 테이블명 별칭
...
SELECT EMP_ID, EMP_NAME, J.JOB_CODE, JOB_NAME -- 어떤 테이블 커럼 조회할지 별칭 작성
...
WHERE DEPT_CODE = DEPT_ID(+);
-- DEPT_CODE의 값이 DEPT_ID와 일치하지 않아도 결과에 포함해라 (추가)
...
-- DEPATRMENT의 D3,D4,D7도 JOIN 결과에 포함!! (기준이 DEPARTMENT이기 때문)
...
-- EMPLOYEE 테이블 하동운, 이오리
-- DEPARTMENT 테이블의 D3, D4, D7
-- 모두를 JOIN 결과에 포함 시켜라
-- == LEFT UNION(합집합) RIGHT 한 결과와 같다

---------------------------------------------------------------------------------------------------------------

-- 3. 교차 조인(CROSS JOIN == CARTESIAN PRODUCT)
--  조인되는 테이블의 각 행들이 모두 매핑된 데이터가 검색되는 방법(곱집합)
--> 직접 사용보단 NATURAL JOIN의 실패 결과로 보여지는 모습이다 ! 라고 알아두면 좋음
...
-- 범위의 급여가 맞을 때에만 결과에 포함하겠다는 JOIN
--> 사원이 자신의 급여 레벨에 맞는 돈을 잘 받고 있는지 확인하기
...
-- 같은 테이블을 조인.
-- 자기 자신과 조인을 맺음
--> 똑같이 생긴 테이블 2개 있다고 생각하면 쉽다!!!

-- EMPLOYEE 테이블에서
-- 모든 사원의 이름과 사수번호 (MANAGER_ID), 사수이름 조회 
...
FROM EMPLOYEE
NATURAL JOIN JOB
--> 두 테이블에서 같은 컬럼명, 자료형을 지닌 JOB_CODE 컬럼 기준으로 JOIN 진행
-- (생각보다 많이 안 쓰게 됨)

/*NATURAL JOIN 실패
 * -> 같은 컬럼명, 자료형이 없을 경우 JOIN 실패
 *  --> 결과로 CROSS JOIN 형태가 조회
 * */
 SELECT EMP_NAME, DEPT_TITLE
 FROM EMPLOYEE
...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE) -- NATURAL도 가능
LEFT JOIN DEPARTMENT ON (DEPT_CODE=DEPT_ID) -- NULL인 친구들이 80년생일 수 있어서 포함해야 함
...
--연봉에 보너스포인트를 적용하시오.
SELECT EMP_NAME, JOB_NAME, SALARY "급여", (SALARY * 12) * NVL(BONUS,0) "연봉(보너스 포함)"  
FROM EMPLOYEE E
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
...
SELECT EMP_NAME 사원명, DETP_TITLE 부서명, LOCAL_NAME 지역명, NATIONAL_NAME 국가명
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
