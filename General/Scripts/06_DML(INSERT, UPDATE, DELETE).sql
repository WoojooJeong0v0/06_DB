...
-- SELECT도 DML 에 포함되긴 하지만 DQL 이라 분리해서 보기도 한다!!
...
-- 테이블 복제해서 만들 것
-- 서브쿼리 결과로 해서 테이블을 새롭게 만든다

SELECT * FROM EMPLOYEE2;
SELECT * FROM DEPARTMENT2;
-- 테이블이 인식되지 않아서 붉은 색으로 처리, 이 경우 SCHEMAS 에서 테이블 새로고침
...
SELECT COUNT(*) -- 전체 행의 개수 23행 카운트하기
FROM EMPLOYEE2;
...
AS SELECT * FROM EMPLOYEE; -- 23행 -- 이미 만들어졌다고 안 나오는데?
...
VALUES (999, '곽두원', '561016-1234567', 'kwack_dw@kh.or.kr',
        ...
        SYSDATE, NULL, DEFAULT); -- 5행
       
SELECT * FROM EMP_M01; 
SELECT * FROM EMP_M02;

 --> EMP_M02 는 M01 테이블과 4개의 행이 중복 
 -- 한 개의 행이 다른 걸 알 수 있음 (곽두원 차이)

UPDATE EMP_M02 SET SALARY = 0;

SELECT * FROM EMP_M02;

MERGE INTO EMP_M01 USING EMP_M02 ON(EMP_M01.EMP_ID = EMP_M02.EMP_ID) -- 중복 확인용 KEY 값
...
WHEN NOT MATCHED THEN -- 중복되지 않으면 
INSERT  -- 삽입할 거야
...
-- [작성법]
-- DELETE FROM 테이블명 WHERE 조건설정  -- SELECT와 비슷
