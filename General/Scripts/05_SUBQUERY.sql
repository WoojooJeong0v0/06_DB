...
-- 2) 부서별 급여합이 21760000원 부서의 부서명과 급여 합 조회
	...
	FROM EMPLOYEE
 	LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
 	GROUP BY DEPT_TITLE                     -- 부서 이름으로 그룹 지었고 결과 SELEC절로 나오는 건 그룹명뿐
 	HAVING SUM(SALARY) = 2176000;
...
-- 3) >> 위의 두 서브쿼리 합쳐 부서별 급여 합이 큰 부서의 부서명, 급여 합 조회
	...
	FROM EMPLOYEE
 	...
 	( SELECT MAX(SUM(SALARY))
 		FROM EMPLOYEE
 		GROUP BY DEPT_CODE ); -- 서브쿼리 결과 21760000원
  ...
  -- 부서별 인원 수가 3명 이상인 부서의
 	...
 	-- 왜 HAVING?
 	-- 그룹으로 묶인 부서에 대한 조건이라서
 	-- HAVING COUNT(*) > = 3
  
  -- 부서별 인원 수가 가장 적은 부서의
 	-- 부서명, 인원 수 조회
  	SELECT NVL(DEPT_TITLE, '없음') 부서명, COUNT(*) 인원
 		FROM EMPLOYEE
 		LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
 		GROUP BY DEPT_TITLE
  	HAVING COUNT(*) = 
   ...
   -- 서브쿼리 결과 2
  	...
  	/* **************** 서브쿼리에서 사용한 별칭을 메인 쿼리에서 사용하기 (아주 중요!!!!!!!!) */
  	-- 인라인뷰 사용
  	-- 인라인 뷰 : FROM절에 사용된 서브쿼리
  			-->  서브쿼리 결과가 테이블처럼 인식이 될 것
		SELECT "이름", "급여"
  	FROM (SELECT EMP_NAME "이름", SALARY "급여"
					FROM EMPLOYEE)
		WHERE "급여" > 4000000;
		-- 메인쿼리 해석 1	순위인 FROM절에 작성된
		--> 서브쿼리 결과 컬럼명이 "급여"이기 때문에
		-- 메인쿼리 해석 2 순위인 WHERE
		-- 메인쿼리 해석 3 순위인 SELECT 절에서도
...
-- 1) 사수에 해당하는 사원 번호 조회
		--> MANAGER_ID 컬럼 작성되어 있는 사번 가진 사원 == 사수 
...
-- 방법1) UNION 이용 방법
		-- UNION : 두 SELECT의 결과 (RESULT SET) 하나로 합침
	SELECT EMP_ID, 
	EMP_NAME, 
	NVL(DEPT_TITLE, '없음'), 
	JOB_NAME,
...
-- 방법2) * SELECT 절에도 서브쿼리 사용할 수 있음
	...
	SELECT  EMP_ID, 
					...
					CASE -- 선택함수 괄호 열고
					--	WHEN 조건
					--	THEN 조건이 TRUE인 경우 수행
						WHEN EMP_ID IN (SELECT DISTINCT MANAGER_ID -- 1번)
														FROM EMPLOYEE
														WHERE MANAGER_ID IS NOT NULL)
						THEN '사수'
						ELSE '사원'
					END AS "구분" -- 선택함수 괄호 닫고
	FROM EMPLOYEE
	JOIN JOB USING(JOB_CODE)
	LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
	ORDER BY EMP_ID ASC;
...
FROM EMPLOYEE
WHERE ENT_YN = 'Y'
AND SUBSTR(EMP_NO, 8, 1) IN ('2', '4');  -- 퇴사여부 = 'Y' == 퇴사한 직원
-- 이태림 JOB_CODE = 'J6'
-- 이태림 DEPT_CODE = 'D8'
...
-- 1) 급여를 200, 600만 받는 직원 (300만, 700만이 평균급여라 생각 할 경우)
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
-- 보통의 SELECT 문은 서브쿼리 - > 메인 쿼리 순서로 해석하는데 
-- 상관 서브 쿼리는 메인 쿼리 1행 해석 -> 서브쿼리 수행 -> 다시 메인쿼리 1행 해석 -> 서브쿼리 수행 
...
WHERE SALARY > (
	...
	WHERE SUB.JOB_CODE = MAIN.JOB_CODE
	 -- 먼저 해석된 메인쿼리 1개행에
	 -- JOB_CODE 값을 얻어와서 서브쿼리 해석에 사용
);

-- 일상생활에서 가장 많이 쓰이는 녀석!!! 아주 중요함!!!!
...
WHERE EXISTS( -- 서브쿼리 조회결과가 있으면 TRUE == 해당 행을 결과에 포함 
	SELECT '있음' -- 서브쿼리에서 조회되는 컬럼 값은 중요한 게 아님!!
		-- 조회되는 데이터가 있는지 없는지 중요
	FROM EMPLOYEE "SUB"
...
-- NVL을 MANAGER_ID에 입히면 실행이 안 됨
-- 서브쿼리에서 MANAGER_ID 값을 가져와야 해서 그런 건가?


-----------------------------------------------------------------------


-- 7. 인라인 뷰(INLINE-VIEW) -- SELECT 안에 작성되어있다는 느낌 
			--> SELECT 문에서 조회되는 가상 테이블

/* VIEW  라는 객체 : 조회 용도의 가상 테이블
...
FROM EMPLOYEE
WHERE ROWNUM <= 5; -- FROM 해석한 시점에서의 행 번호
...
FROM 
	...
	FROM EMPLOYEE
	ORDER BY SALARY DESC
	) -- > 서브쿼리 결과 (23행 2열)를 메인쿼리의 테이블로 인식 시킴
	--> 급여 내림차순으로 정렬되어 있는 상태
	--> 미리 정렬 시킨 테이블을 마련하는 것
	--> 이 서브쿼리를 조회용으로 써서 인라인 뷰라고 함
WHERE ROWNUM <= 5;
...
FROM
	 ...
	 ORDER BY "급여 평균" DESC)
WHERE ROWNUM <= 3; -- 서브쿼리 조회결과에서 1, 2, 3 행만 조회하겠다
 ----------------- 위에 작성된 거 왜 안 되는지 확인 필요 
SELECT
	ROWNUM "순위",
	"부서 코드", "부서명", "급여 평균"
...
-- 1) FROM절에 서브쿼리 인라인 뷰 직접 작성하는 일반적 방식
SELECT 
	ROWNUM "순위", 
	EMP_NAME "이름", 
	SALARY "급여"
...
-- 1) RANK() OVER : 
-- OVER () 에 작성된 정렬 기준대로 정렬 후 순위 지정함
-- 단, 값의 크기가 같다면 공동순위로 지정되며
-- 지정된 만큼 후순위 건너뛰기 진행 
...
-- 항상 범위에 1부터 연속적인 범위가 포함되어야만 한다 (아니면 결과가 안 나옴)
-- --> ROWNUM 은 RESULT SET 완성 후 적용되는 가상컬럼이기 때문에
--     가상컬럼이 만들어지는 정해진 규칙 (1부터 1씩 증가) 을 만족하지 못하면 사용 불가 
...
FROM
	( SELECT 
	 ...
	 FROM EMPLOYEE)
WHERE 
 ROWNUM BETWEEN 3 AND 7; -- 3행이상 7행 이하  - 불가
-- ROWNUM BETWEEN 1 AND 7; -- 3행이상 7행 이하  - 가능
...
-- 사번, 사원명, 전화번호, 고용일, 부서명
 ...
 SELECT EMP_NAME, EMP_ID, PHONE, HIRE_DATE, DEPT_CODE
 FROM EMPLOYEE
 LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
 WHERE DEPT_CODE =
 (	SELECT DEPT_CODE
...
-- 고용일 2010년 이후 사원들 급여순으로 조회
 ...
 FROM EMPLOYEE
 JOIN JOB USING (JOB_CODE)
 WHERE HIRE_DATE LIKE '1%'
 ORDER BY SALARY DESC;

-- 실무에서 USING 사용 못하는 경우가 많음 !! 
...
SELECT *
FROM EMPLOYEE
WHERE EXTRACT(YEAR HIRE_DATE) >= 2010; -- 년도까지 잘라서 보고 비교
...
-- 사번, 이름, 부서코드, 직급코드, 부서명, 직급명
SELECT EMP_ID, EMP_NAME, JOB_CODE, DEPT_CODE, JOB_NAME
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE DEPT_CODE = 
	(	SELECT DEPT_CODE
...
WHERE HIRE_DATE IN (   -- 괄호 안의 값이 HIRE-DATE와 같은지 봄
	SELECT MIN(HIRE_DATE)
 	FROM EMPLOYEE
 	WHERE ENT_YN != 'Y'
 	GROUP BY DEPT_CODE -- 22행 중에서 그룹을 짓게 됨
  )
 ORDER BY HIRE_DATE ASC;
 
 -- 컬럼은 하나인데 행이 여러 개 = 다중행 서브쿼리 
...
-- 정확하지 않은 방법에 대해 수업 내용 다시 듣기 필요!! 


 	-- 2) 상관 서브 쿼리 이용 (정확도 높음!)
...
WHERE HIRE_DATE IN 
	...
	FROM EMPLOYEE "SUB"
	WHERE ENT_YN != 'Y'
	AND NVL("SUB".DEPT_CODE, '소속없음') = NVL(E.DEPT_CODE /*'D1'*/, '소속없음'))
ORDER BY 
	HIRE_DATE ASC;
--- 강사님 답 
SELECT 
...
GROUP BY JOB_CODE;
-- 정확하게 하려면 TO_DATE 
-- 행이 7개니까 다중행 서브쿼리로 이용

-- 2) 1)번 결과를 다중행 서브쿼리로 사용해서 생일이 일치하는 사원들 조회 진행
SELECT 
	EMP_ID,
	EMP_NAME,
	JOB_NAME,
...
JOIN JOB      J ON (E.JOB_CODE = J.JOB_CODE)
WHERE TO_DATE(SUBSTR(EMP_NO, 1, 6)) 
	= (  -- IN 도 가능하지만 비교연산 = 으로 바꿔주면 좋다
		SELECT MAX( TO_DATE(SUBSTR(EMP_NO, 1, 6)) )
		FROM EMPLOYEE "SUB"
		WHERE "SUB".JOB_CODE = E.JOB_CODE)
		-- 현재 메인쿼리의 해석되는 행이 해당 직급에서 가장 어린 사원이 맞는지 비교
ORDER BY "만 나이" DESC;